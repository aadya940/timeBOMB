...
class TaskManager:
    ...
    #     echart["series"][0]["data"][0]["value"] = self.completed_tasks
    #     echart["series"][0]["data"][1]["value"] = self.incomplete_tasks
    #     if echart_element is not None:
    #         echart_element.update()
    ...
    def process_expired_tasks(self):
        ...
        def check_queue():
            try:
                ...
                print(f"Processing expired task: {task['name']}")
                self.ask_completion(task)
            except queue.Empty:
                print("No expired tasks to process.")

        ui.timer(0.5, check_queue)
...
@ui.page("/analytics")
def _analytics():
    # Initialize echart_element early
    echart_element = (
        ui.echart(
            {
                "title": {
    ...
    ui.timer(2, echart_element.update)
...
def home():
    ...
    with ui.column().classes("items-center") as column_element:
        with ui.row().classes(
        ...
        with ui.element("div") as notify_element:
            pass

        notify = notify_element
    ...
    with ui.dialog() as add_task_dialog:
        with ui.column().classes("p-4 items-center") as main_col:
            ...
            time_input = ui.input(
                ...
                value="12:00 AM",
                validation={
                    "Improper Time (Hours).": lambda value: (
                        value is None
                        or (
                            len(value.strip()) >= 7
                            and (
                                value.strip()[0:2] in [str(i) for i in range(0, 13)]
                                or value.strip()[0:1] in [str(i) for i in range(0, 13)]
                            )
                        )
                    ),
                    "Improper Time (Minutes).": lambda value: (
                        value is None
                        or (value.strip()[3:5] in [str(i) for i in range(0, 61)])
                    ),
                    "Improper AM/PM.": lambda value: (
                        (value is None) or (value.strip()[-2:].upper() in ["AM", "PM"])
                    ),
                },
            )
